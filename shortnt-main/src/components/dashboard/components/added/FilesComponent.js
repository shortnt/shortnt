...
const FilesComponent = (props) => {
  ...
  useEffect(() => {
    fetch(process.env.REACT_APP_API + 'files/' + "f" + props.data.folder_id)
      .then((res) => res.json())
  ...
  const getAudio = () => {
    console.log(props.data.folder_id)
  ...
  const getSummary = (event) => {
    ...
    fetch(process.env.REACT_APP_API + 'files/', {
      ...
      body: JSON.stringify({
        file_id: null,
        file_title: filename,
        folder_id: "f" + props.data.folder_id.toString(),
  ...
  return (
    <>
      ...
      <Page
        ...
        breadcrumbs={[{ name: ' ' + props.data.folder_title, active: true }]}
      >
        <h1 style={{ textAlign: "center" }}>{props.data.folder_title}</h1>
